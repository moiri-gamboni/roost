...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `agents/` -- Subagent definitions deployed to `$CLAUDE_CONFIG_DIR/agents/`. `effort-high.md` is a **config shim** carrying `effort: high` and nothing else (`name`/`description` are required by the format; the body is a placeholder because it becomes the subagent's entire system prompt). It exists because the `Agent` tool passes `model` per invocation but not `effort`, making frontmatter the only per-subagent effort lever — so one file serves every tier and the model is chosen at the call site. Overridden by `CLAUDE_CODE_EFFORT_LEVEL`, which is therefore deliberately **absent** from `settings.json`: setting it collapses every tier to one effort level
...
│   ├── skills/             Skills
│   ├── agents/             Subagent definitions (effort-high — effort level only)
