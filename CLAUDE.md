...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `settings.json` -- Claude Code settings. **Runtime-rewritten**: the app writes `/model` and `/config` choices straight back into the live file, so the repo copy tracks live rather than dictating it — never blanket `roost-apply push` (it would revert whatever the repo has not learned yet), and compare with `jq -S` since the app's key reordering makes the textual diff meaningless. Also holds hook definitions (Notification + perm-mark; UserPromptSubmit usage line + turn start, plus the asyncRewake auto-resume waiter entry; Stop/StopFailure/SessionEnd/Subagent*/PostCompact lifecycle events; PostToolUse shellcheck), Notion permissions gate, cleanup policy
...
| UserPromptSubmit (second entry, `asyncRewake: true`, timeout 700000) | `scripts/session.sh --rewake-waiter` | The **auto-resume waiter**. asyncRewake makes the harness background the hook and treat exit 2 as "wake the model with stderr as a system reminder", delivered through the same task-notification channel as a finished background Bash task — it wakes even an idle session (verified end-to-end on 2.1.233). Below `USAGE_WARN_PCT` every spawn exits 0 immediately; once a window warns, the first spawn per session becomes the waiter (pidfile `usage/sessions/<sid>.rewaiter`, newest-wins dedupe), sleeps to the earliest warned reset in ≤300s chunks, and exits 2 at the reset — or within ≤5m of a login switch (`session account use`), since the switch itself supplies the fresh window. Exits 0 without waiting under a `claude -p` parent (ancestor argv walk): the harness only backgrounds asyncRewake when `isInteractive()\|\|hasStreamingInput()`, so under `-p` the hook runs synchronously and a wait would block the print run until the reset. `rewakeMessage`/`rewakeSummary` in settings.json set the reminder prefix and the user-visible terminal line. |
