...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `laptop/` -- Scripts and systemd units designed to run on the laptop, not the server. Each component has a self-contained `install-*.sh` that reads `.env` and handles install + unit rendering + enable in one step.
    - `guest-bootstrap.sh` -- One-shot provisioning of a **guest device** for an isolated Claude Code session (academic-research kit: Chrome, fnm/Node, uv, Go, rodney, html2markdown, mmdc, roughdraft, showboat, pandoc, plus the credential-free skills html2markdown/havelock-api/humanizer). Public tools only — no roost credentials, no MCP servers, no tailnet; auth is a manual `/login` as the designated guest account (account restriction holds because switching accounts needs a fresh browser authorization). Runs from a checkout or standalone via curl from the public repo (Ubuntu x86_64). Rationale + alternatives: `plans/isolated-secondary-session-ideation.md` (local, gitignored).
