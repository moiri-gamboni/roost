...
| PreToolUse (Bash) | `hooks/notion-write-guard.sh` | Turns an ad-hoc REST write to `api.notion.com` (write verb + host in the raw command) into a permission prompt; `apart-tools/tasksync/` and `apart-tools/notion-mirror/` invocations pass, as do the POST-shaped reads (`/query`, `/v1/search`) when they are the only Notion paths in the command. Approve to run it as is; decline and the session routes through tasksync or the MCP tools. Where nothing can prompt, ask is a deny |
