...
- **`files/`** — everything deployed to the server (detail: `files/CLAUDE.md`)
  ...
  - `private/` — separate git repo (`roost-private`, gitignored here, deployed via the public manifest): `global-CLAUDE.md` (→ `~/roost/claude/CLAUDE.md`), `cron-mirrors`, `claude-plugins.sh`, `drive-mirror-refresh.sh`, personal Caddy sites. Commit there, then `roost-apply push`
  ...
  - `beeper/` — the attention queue's Beeper Server: egress allowlist script + hosts, `beeper-egress`/`beeper-server`/`attention-bridge@` units (`files/beeper/CLAUDE.md`; runbook `docs/runbooks/attention-queue.md`)
  - `hooks/` — Claude Code event hooks → `$CLAUDE_CONFIG_DIR/hooks/` (`files/hooks/CLAUDE.md`)
  - `scripts/` — user CLIs → `claude/scripts/`, symlinked into `~/bin`: `roost-apply`, `roost-net`, `agent-worktree`, `conflict-watch` (`files/scripts/CLAUDE.md`)
  - `conflict-watch.conf` + `conflict-watch.service` — the conflict watch's unit rules and root daemon (`files/CLAUDE.md`, mechanism in `files/scripts/CLAUDE.md`)
  ...
  - `setup/` — modular setup scripts run by `deploy.sh` via `remote_script()`: `system`, `create-user`, `ssh-hardening`, `ufw`, `swap`, `snapper`, `tailscale`, `shell-config`, `dev-tools`, `caddy`, `ntfy`, `cloudflare`, `privatebin`, `travel-vpn`, `dufs`, `glances`, `ram-monitor`, `earlyoom`, `cron`, `claude-code`, `claude-config`, `agent-tools`, `et`, `clip-forward`, `attention-queue` (beeper user, nested subvolumes, pinned Beeper Server, unit enables), `unattended-upgrades`
...
│   ├── hooks/              Event hooks (notify, statusline, shellcheck-edit, notion-write-guard, beeper-send-guard, truncation-guard (unwired), fork-context-guard, redelegation-guard, subagent-context, agent-browser-session, conflict-watch-hook) + session-switch-notify (the session CLI's push on a login switch)
...
|---|---|---|
| Notification | `hooks/notify.sh` | ntfy push for permission prompts, titled with the session's name (rate-limited) |
...
| PreToolUse (every tool) · PostToolUse(+Failure) (writing tools) · UserPromptSubmit | `hooks/conflict-watch-hook.sh` | The conflict watch, session side. When another open session holds a unit (a task folder, a plans/notes/data entry, a whole repo) because it wrote there: an Edit/Write into it asks the user (holder named, busy/idle, last write); the first Bash command naming it is denied (the retry passes: reading is fine); a repo-wide git command asks if the holder has uncommitted work there; writes that happen anyway (seen by the daemon) come back as "stop and ask the user", and the holder hears about them too. The model's options: ask the user, SendMessage the holder, `agent-worktree isolate <repo>` for code; `conflict-watch release` when done in a unit. Silent while the daemon is down |
...
| PreToolUse (Bash) | `hooks/beeper-send-guard.sh` | Turns a Bash command that would send a message to a person through the local Beeper Server (`127.0.0.1:23373`) into a permission prompt naming the chat, its network and the people in it: `aq send`, an import of the Beeper SDK, and a raw API write (curl `-X POST`/body flags, `.post(`-class calls) whose chat lookup shows a person, or whose destination cannot be read or looked up. Reads, sends to bot DMs (Slackbot, app bots), bridge control chats and the solo board chat pass. Approve to send; where nothing can prompt, ask is a deny. A gate against ordinary and accidental sends, not a boundary: a script on disk passes |
...
- **beeper-server**, **attention-bridge@slack**, **beeper-egress** (+ `beeper-egress-ensure.timer`, 5 min) — the attention queue: headless Beeper Desktop as the `beeper` user behind a default-deny egress allowlist, the self-hosted Slack bridge as the user via `bbctl`, the pass every 15 min from `cron-mirrors`, and a dead-man in the private health checks. `files/beeper/CLAUDE.md`; runbook `docs/runbooks/attention-queue.md`.
- **conflict-watch** — root daemon recording which open Claude Code session writes in which unit (fanotify close-after-write on the root mount, notification events only, plus the proc connector for short-lived writers), feeding `hooks/conflict-watch-hook.sh`. Fails silent (nothing watched, nothing blocked) and the health check alerts when it is down. State in `/run/conflict-watch`, kept across a restart.
...
| Command | Usage |
|---|---|
| `agent [path] [claude-args...]` | Launch interactive Claude in a tmux window, working directly in the directory (path defaults to cwd). Sessions share the live folders; the conflict watch stops one before it edits where another is working, and `agent-worktree isolate` gives it a worktree of that one repo then |
| `agent -c` | Continue last session in cwd (a worktree session resumes into its own) |
| `agent -w` | Launch in a composite worktree of the repo and every nested repo (`agent-worktree`, below). `git config agent.noWorktree true` on a *nested* repo means "share me live into every session tree" instead of a per-session checkout (the Notion mirror at `~/roost/apart-research/mirrors/notion`). `-N`/`--no-worktree` is accepted and does nothing: running in the directory is the default |
...
- Hooks source `lib/_hook-env.sh` for `hook_input()`/`hook_json()`, `ntfy_send()` (journald fallback), `rate_limit_ok()`, `logger -t roost/<script>`. Exceptions: `reflect.sh` (just cats a prompt), `session --hook` (the session CLI reused as a hook), `notion-write-guard.sh`, `beeper-send-guard.sh`, `fork-context-guard.sh` and `redelegation-guard.sh` (fire on every Bash or Agent call, as `truncation-guard.sh` does when wired; sourcing costs a `tailscale ip` subprocess they can't afford), `subagent-context.sh` (two jq calls, one line of output).
