...
- `health-check.sh` -- Checks Ollama, Caddy, ntfy, Tailscale, cloudflared, disk; hard failures bundle into a high-priority `Service health alert`. Soft signals (sustained swap >3GB high-priority, pending reboot via `/var/run/reboot-required` default-priority) send their own ntfy with per-event cooldowns (swap: 1h; reboot: 7d reminder, re-arms on new mtime). Sources `health-check-apps.sh` if present for app-specific checks.
