...
| App health checks | `~/roost/claude/scheduled/health-check-apps.sh` | Sourced by `health-check.sh`; same `check()`/`check_service()` helpers; deployed from `files/travel/travel-health.sh`, so append there |
