...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `private/` -- Separate git repo (`claude-mds`); commit changes there, then deploy with `roost-apply push`
    ...
    - `granola-digest.sh` -- Personal brief of new/changed Granola notes (the prompt names my role/priorities, hence private). Feeds `claude -p --model claude-sonnet-5 --effort max` (1h timeout — max effort over full transcripts takes 10-20min) the **untruncated** `TODO.md`, the `workflows/` meeting-note-extraction procedure + transcript-corrections glossary, and each changed note in full: Granola's AI summary (flagged **unreliable**) + the verbatim transcript as source of truth. The prompt stays **minimal and defers to the procedure** (single source of truth — editing the workflow changes the digest). Ends with `Reliability:` (uncertainties/garbles, each High/Med/Low) and `Glossary additions:` (proposed rows for `transcript-corrections.md`; I apply them — only a human confirms a High). Pushes **plain text** via ntfy (phone apps don't render markdown), archives to `apart-research/updates/granola/<date>.md`. Deployed to `claude/scripts/`, so `granola-refresh` picks it up
  ...
  - `hooks/` -- Claude Code **event hooks** (deployed to `$CLAUDE_CONFIG_DIR/hooks/`, wired in `settings.json`): `notify.sh` (Notification), `statusline.sh` (TUI status line; also persists the shared usage cache + per-session cost samples under `~/roost/claude/usage/` for `roost-usage`), `reflect.sh` + `reflect.md` (PreCompact, disabled)
  - `scripts/` -- User **CLIs**, symlinked into `~/bin` (deployed to `claude/scripts/`)
    ...
    - `roost-usage.sh` -- Live 5h/weekly usage limits + context fill, plus per-session attribution: `usage session [id]` (invoking session resolved via `roost-session --id`) and `usage sessions`. Per-session $ burn is **counted** from the statusline's cost samples (`usage/session-log.tsv`); est % of limit = the global %-movement observed while sampling was live, split by tracked-$ share (pre-coverage burn stays unattributed; headless `claude -p` and off-box usage are invisible and inflate shares); `usage`/`roost-usage`
    ...
    - `granola-refresh.sh` -- `granola sync` → `granola-transcripts sync` → optional `granola-digest`; ntfys (3-day cooldown) on exit-3 (MCP re-auth needed) while summaries keep working. `granola-refresh [--commit] <mirror-dir>` (or `$GRANOLA_MIRROR`). `--commit` commits just the mirror + that day's brief via a **pathspec commit**, so anything else staged in that repo is untouched. Driven by the private `cron-mirrors` job, which passes `--commit`
...
| UserPromptSubmit | `scripts/roost-usage.sh --hook` | Per-turn: injects a compact, self-identifying one-line `Claude usage limits · date/time · 5h % · weekly %` (+ live reset countdowns) into the model's context via `hookSpecificOutput.additionalContext`, with `suppressOutput` keeping it out of the user's transcript (`timeout: 2`). The leading tag is needed because `additionalContext` reaches the model as a bare string (only the generic `UserPromptSubmit hook` wrapper labels it). Near a hard cap (>= `USAGE_WARN_PCT`, default 90) it also appends a ⚠ advisory pointing at a background `roost-usage --wait 5h` to auto-resume after the reset. The line ends with `· this session ≈X%/5h ≈Y%/wk` — the invoking session's tracked-share estimate (session id read from the hook's stdin JSON). Always exits 0, so it can never block or erase a prompt. |

All these scripts source `_hook-env.sh` from `../lib/` (except `reflect.sh`, which just cats a prompt file, and the UserPromptSubmit hook, which is the `scripts/roost-usage.sh` CLI reused as a hook) which provides `hook_json()` for parsing Claude Code's JSON input, `ntfy_send()` for notifications (with journald fallback), `rate_limit_ok()` to prevent notification floods, and journald logging via `logger -t "$_HOOK_TAG"` (tags: `roost/<script-name>`).
