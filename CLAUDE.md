...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `scripts/` -- User **CLIs**, symlinked into `~/bin` (deployed to `claude/scripts/`)
    ...
    - `roost-usage.sh` -- Live 5h/weekly usage limits + context fill, plus per-session attribution: `usage session [id]` (defaults to the invoking session) and `usage sessions`. Per-session $ burn is **counted** from the statusline's cost samples (`usage/session-log.tsv`); est % of limit = the global %-movement observed while sampling was live, split by tracked-$ share (pre-coverage burn stays unattributed; headless `claude -p` and off-box usage are invisible and inflate shares). **Multi-call**: `~/bin/roost-session` symlinks to this same script — under that name (or as `usage whoami`) it prints the invoking session's id + auto-title (pane-safe: `$CLAUDE_CODE_SESSION_ID` + ancestor-environ walk); `usage`/`roost-usage`/`roost-session`
