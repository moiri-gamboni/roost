...
- Hooks source `lib/_hook-env.sh` for `hook_input()`/`hook_json()`, `ntfy_send()` (journald fallback), `rate_limit_ok()`, `logger -t roost/<script>`. Exceptions: `reflect.sh` (just cats a prompt), `session --hook` (the session CLI reused as a hook), `notion-write-guard.sh`, `truncation-guard.sh`, `fork-context-guard.sh` and `redelegation-guard.sh` (fire on every Bash or Agent call; sourcing costs a `tailscale ip` subprocess they can't afford), `subagent-context.sh` (two jq calls, one line of output).
