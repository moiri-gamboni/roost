...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `private/` -- Separate git repo (`roost-private`); commit changes there, then deploy with `roost-apply push`
    ...
    - `claude-plugins.sh` -- Claude Code plugin installs (the apart-tools marketplace + plugin: state-guarded `claude plugin marketplace add` from the clone, `install --scope user`, enable, update, autoUpdate=true written into `known_marketplaces.json` via jq — no CLI surface for that key); sourced by a generic source-if-present hook in `setup/claude-config.sh`, so plugin specifics stay out of the public repo
