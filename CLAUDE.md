...
| PreToolUse (Bash + Write\|Edit) | `hooks/tasksync-write-guard.sh` | Denies hand-writes to what the `tasks` CLI owns in the apart-research workspace (worklog stint headers/Totals, `identity.json`, `tasks/.sync/`); in-place corrections of an existing header pass. Deny names the sanctioned verb (`tasks stint` / `tasks pull`). Friction, not a boundary |
