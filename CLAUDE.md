...
- `deploy.sh`, `test-server.sh`; `plans/` design docs; `reviews/`; `docs/runbooks/`; `tests/` (`agent-worktree.sh`, `agent-launch.sh`, `bashrc-reload.sh`, `conflict-watch.py`, `conflict-watch-daemon.sh`, `conflict-watch-e2e.sh`, `ram-monitor.sh`)
- `files/` — everything deployed to the server; per-file detail in `files/CLAUDE.md`
  ...
  - `scripts/` → `~/roost/claude/scripts/`, symlinked into `~/bin`: `roost-apply`, `roost-net` (travel VPN), `agent-worktree` (composite worktrees for `agent -w` and single-repo ones via `agent-worktree isolate`, wired as the `WorktreeCreate`/`SessionEnd` hooks; its header is the contract), `conflict-watch` (the conflict watch's daemon and CLI: `status`, `release`, `allow`, `unit`)
  ...
  - `claude-plugins/` → `~/roost/claude/roost-plugins/` — the local `roost` plugin marketplace (a directory source, loaded in place): `bash-lsp`, the Bash language server for Claude Code's LSP tool; Python and TypeScript use the official `pyright-lsp`/`typescript-lsp`. Binaries from `setup/dev-tools.sh`, plugins from `setup/claude-config.sh`; detail in `files/CLAUDE.md`
  - `conflict-watch.conf` → `~/roost/claude/conflict-watch.conf` — the conflict watch's unit rules (which folders count as one unit, which trees are ignored)
...
The `session` tool (identity, rate-limit usage, time tracking, login switching, reboot survival) lives in its own clone at `~/roost/code/session`, whose README is the ops contract; this repo only deploys the conf, tmux hooks, focus-tick timer and boot unit that feed it (`files/scripts/CLAUDE.md`). **`sym`** (whole definitions and their callers by name, the code-context tool the global CLAUDE.md points sessions at instead of Grep plus a guessed line count) lives in its own clone at `~/roost/code/sym` (public, github.com/moiri-gamboni/sym), cloned and linked into `~/bin` by `private/claude-plugins.sh`; this repo installs only its engine, the `ast-grep` release binary (`setup/dev-tools.sh`, refreshed by the auto-update), never the npm package, whose `sg` would shadow `/usr/bin/sg`.
...
- **conflict-watch** — root daemon (`files/conflict-watch.service`, `setup/conflict-watch.sh`) recording which open Claude Code session writes in which unit (fanotify close-after-write notification events, never permission events, plus the proc connector for short-lived writers); `hooks/conflict-watch-hook.sh` stops a second session before it works in a unit another open session holds. `agent` launches sessions in the live folders and relies on it; the health check alerts when it is down. Design: `plans/session-conflict-watch.md`; mechanism: `files/hooks/CLAUDE.md`, `files/scripts/CLAUDE.md`.
