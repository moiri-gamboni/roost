...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `settings.json` -- Claude Code settings: default model `claude-fable-5[1m]` with availability fallback to `claude-opus-4-8` (`fallbackModel`; Fable's content-flagged requests additionally auto-fall-back to Opus 4.8 natively), hook definitions (Notification; UserPromptSubmit usage line; Stop turn-end), Notion permissions gate, cleanup policy
...
| Stop | `scripts/session.sh --turn-end` | Appends the turn's end event to `usage/turn-log.tsv` (paired with the UserPromptSubmit start) for `session time` per-turn tracking. Always exits 0 with no output — a Stop hook's exit 2 would force the agent to keep going. |
