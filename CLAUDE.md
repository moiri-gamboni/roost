...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `private/` -- Separate git repo (`roost-private`); commit changes there, then deploy with `roost-apply push`
    ...
    - `cron-mirrors` -- Private cron fragments (personal data-mirror refresh jobs: notion-mirror + tasksync pull/drain — both running from the `apart-research/apart-tools/` clone — plus granola-mirror, drive-mirror); envsubst-rendered and deployed to `/etc/cron.d/$ROOST_DIR_NAME-mirrors` (job specifics stay in the private repo)
