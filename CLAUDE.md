...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `settings.json` -- Claude Code settings. **Runtime-rewritten**: the app writes `/model` and `/config` choices straight back into the live file, so the repo copy tracks live rather than dictating it — never blanket `roost-apply push` (it would revert whatever the repo has not learned yet), and compare with `jq -S` since the app's key reordering makes the textual diff meaningless. Also holds hook definitions (Notification + perm-mark; UserPromptSubmit usage line + turn start, plus the asyncRewake auto-resume waiter entries on UserPromptSubmit and StopFailure; Stop/StopFailure/SessionEnd/Subagent*/PostCompact lifecycle events; PostToolUse shellcheck), Notion permissions gate, cleanup policy, and `autoContinueAtUsageLimit: false` (the built-in 5h auto-continue is off — cancel-on-any-keystroke made it unreliable and its quoted reset time misleads sessions across login switches; the waiter owns auto-resume)
  - `private/` -- Separate git repo (`roost-private`); commit changes there, then deploy with `roost-apply push`
    ...
    - `cron-mirrors` -- Private cron fragments (personal data-mirror refresh jobs: notion-mirror, tasksync pull/drain and granola-mirror — all three running from the `apart-research/apart-tools/` clone — plus drive-mirror); envsubst-rendered and deployed to `/etc/cron.d/$ROOST_DIR_NAME-mirrors` (job specifics stay in the private repo)
    - `claude-plugins.sh` -- Claude Code plugin installs (the apart-tools and diataxis-skill marketplaces + plugins: state-guarded `claude plugin marketplace add`, `install --scope user`, enable, update, autoUpdate=true written into `known_marketplaces.json` via jq — no CLI surface for that key); sourced by a generic source-if-present hook in `setup/claude-config.sh`, so plugin specifics stay out of the public repo. The diataxis plugin (skill `diataxis:diataxis`, auto-triggers on documentation work) ships the verbatim diátaxis.fr text with a routing SKILL.md; source repo `~/roost/code/diataxis-skill` → public `github.com/moiri-gamboni/diataxis-skill` (CC BY-SA 4.0), and content updates reach installed copies only on a version bump (repo pre-commit hook handles it). Praxis routes its documentation work through this skill (implementer/design/review hooks + `/praxis:document`)
    ...
    - `granola-digest.sh` -- Personal brief of new/changed Granola notes (the prompt names my role/priorities, hence private). Feeds `claude -p --model claude-sonnet-5 --effort max` (1h timeout — max effort over full transcripts takes 10-20min) the **untruncated** task list (`tasks/.sync/ls.md`, the offline `tasks ls` view; formerly `TODO.md`, retired 2026-08-10), the meeting-note-extraction procedure (the `/apart-tools:meetings` skill, read from the apart-tools clone at `apart-research/apart-tools/skills/meetings/SKILL.md` — hard-fails if missing) + **both tiers** of the transcript-corrections glossary (reviewed + auto, still in `workflows/meetings/` — the glossary is data, not procedure), and each changed note in full: Granola's AI summary (flagged **unreliable**) + the verbatim transcript as source of truth. The prompt stays **minimal and defers to the procedure** (single source of truth — editing the skill changes the digest). Ends with `Reliability:` (uncertainties/garbles, each High/Med/Low) and `Glossary additions:` (pinned row format), which the script **appends verbatim** under a dated heading to `workflows/meetings/transcript-corrections-auto.md` — the **unreviewed tier**: applied by consumers but never silently (original garble stays visible; reliance flagged by severity per its header), fed back into future digests, and doubling as the lazy review queue from which I promote rows into the reviewed `transcript-corrections.md`. Pushes **plain text** via ntfy (phone apps don't render markdown), archives to `apart-research/updates/granola/<date>.md`. Deployed to `claude/scripts/` + symlinked into `~/bin`, so the apart-tools clone's `granola-mirror/refresh.sh` picks it up as its optional digest step (the granola mirror tooling itself — `granola`, `granola-transcripts`, `refresh.sh`, plus the auto meeting-note step `notes.sh` — moved to `apart-research/apart-tools/granola-mirror/` on 2026-08-19; ops contract in that directory's README)
  ...
  - `notion-webhook.service` -- Systemd unit for the Notion webhook receiver (`apart-research/apart-tools/notion-mirror/webhook_receiver.py`; captured from the previously hand-installed live unit). `Restart=on-failure` does **not** reload on file change: any edit to `webhook_receiver.py` needs `sudo systemctl restart notion-webhook`, and rollback order is revert-the-file *then* restart
  - `granola-webhook.service` -- Systemd unit for the Granola webhook receiver (`apart-research/apart-tools/granola-mirror/webhook_receiver.py`, 127.0.0.1:8097, routed as the `^/granola` path on the hooks hostname via the `granola-hooks.yml` tunnel fragment). Primary trigger for the granola-mirror pipeline: a Standard-Webhooks-verified `note.generated`/`note.edited`/`note.access_granted` event kicks the clone's `refresh.sh --commit`, debounced, under the same flock as the fallback cron. Refuses to start without the signing secret at `~/.config/granola/webhook-secret` (written once at endpoint registration — see the receiver's docstring). Same restart caveat as notion-webhook
  ...
  - `skills/` -- Claude Code skills deployed to `$CLAUDE_CONFIG_DIR/skills/`
    - `codex/SKILL.md` (delegate tasks to OpenAI Codex/GPT as headless subagents via `codex exec` — spends OpenAI credits, not Claude rate-limit budget), `html2markdown/SKILL.md`, `havelock-api/SKILL.md`, `humanizer/SKILL.md`, `pastebin/SKILL.md` (publish encrypted pastes to PrivateBin via pbincli), `roughdraft/SKILL.md` (put a Markdown file in front of the user for review, wait for Done Reviewing, reply in the document; the *usage* half of what used to sit in the global CLAUDE.md — the install/fork half stays there), `session/SKILL.md` (the session CLI: identity + usage limits + per-session attribution), `zotero/SKILL.md` (batch-manipulate a Zotero library: local read API + pyzotero writes + direct storage/pdftotext for bulk PDFs; written for the guest device but deployed here too — the web-API path works headless)
  ...
  - `travel/` -- Travel VPN server pieces (Xray + Proton egress); see Travel VPN section below
    ...
    - `travel-health.sh` -- Deployed as `health-check-apps.sh`; sourced by the base health check (hosts travel-vpn + PrivateBin checks, plus the roughdraft-write-guard liveness probe)
...
| UserPromptSubmit | `scripts/session.sh --hook` | Per-turn, **warn-gated**: silent (no context injected) while both rate-limit windows are below `USAGE_WARN_PCT` (default 90). At/above it, injects a compact, self-identifying one-line `Claude usage limits · date/time · 5h % · weekly %` (+ live reset countdowns) into the model's context via `hookSpecificOutput.additionalContext`, with `suppressOutput` keeping it out of the user's transcript (`timeout: 2`). The leading tag is needed because `additionalContext` reaches the model as a bare string (only the generic `UserPromptSubmit hook` wrapper labels it). It appends **one** ⚠ advisory — a single message even when both windows warn, identical whatever the window or reset distance (only the lead sentence names which window(s) crossed): keep working, fan-out included — a cap is never a reason to stop or wind down — and the auto-resume waiter is already armed (see the `--rewake-waiter` row below), so nothing is asked of the model; when settings.json lacks the asyncRewake entry the advisory falls back to instructing a background `session --wait` launch. The countdown reads `resets to 0% in …` so it can't be mistaken for a work deadline. Once a warned window's `resets_at` passes — or the login switches to an account below the threshold (the warn state records the warning login, so a foreign countdown can't leave the notice sitting silent for days) — the session's next turn gets a one-line refresh notice — no breakdown, just "the window has reset / the login switched, work can continue" — emitted once (per-session warn state `reset-ts + login` in `usage/sessions/<sid>.warn`, swept with the snapshots); unwarned sessions never see it. The line ends with `· this session ≈X%/5h ≈Y%/wk` — the invoking session's tracked-share estimate (session id read from the hook's stdin JSON). Side effect every turn, gated or not: appends the turn's start event to `usage/turn-log.tsv` for `session time`. Always exits 0, so it can never block or erase a prompt. |
| UserPromptSubmit + StopFailure (second entries, `asyncRewake: true`, timeout 700000) | `scripts/session.sh --rewake-waiter` | The **auto-resume waiter**. asyncRewake makes the harness background the hook and treat exit 2 as "wake the model with stderr as a system reminder", delivered through the same task-notification channel as a finished background Bash task — it wakes even an idle session (verified end-to-end on 2.1.233 for UserPromptSubmit; asyncRewake is schema-generic across events). From UserPromptSubmit, below `USAGE_WARN_PCT` every spawn exits 0 immediately; once a window warns, the first spawn per session becomes the waiter (pidfile `usage/sessions/<sid>.rewaiter`, newest-wins dedupe). From StopFailure it arms only when the turn died on a usage cap (error ~ `rate_limit`), and then regardless of the warn threshold — the only path covering a cap that arrives mid-turn with no prompt ever submitted in the warned band. The waiter sleeps to the earliest warned reset (≤300s chunks as a missed-event backstop) and exits 2 at the reset — or within ~1s of a login switch (`session account use`), detected via an inotify watch on the config dir (15s polling without inotifywait), since the switch itself supplies the fresh window. Exits 0 without waiting under a `claude -p` parent (ancestor argv walk): the harness only backgrounds asyncRewake when `isInteractive()\|\|hasStreamingInput()`, so under `-p` the hook runs synchronously and a wait would block the print run until the reset. `rewakeMessage`/`rewakeSummary` in settings.json set the reminder prefix and the user-visible terminal line. |
| Stop · StopFailure · SessionEnd · SubagentStart/Stop · PostCompact · Notification(permission_prompt) | `scripts/session.sh --turn-end` / `--turn-fail` / `--session-end` / `--subagent-start` / `--subagent-end` / `--compact-mark` / `--perm-mark` | Lifecycle event rows (`e f x a z c p`; `s` comes from the UserPromptSubmit hook) into `usage/turn-log.tsv` for `session time`: turn spans + prompt_id, API-failure turn ends (detail=error type, e.g. `rate_limit`), session terminations, subagent spans (paired by agent_id), compactions, and mid-turn permission-wait markers. All exit 0 unconditionally — several of these events treat exit 2 as "block". (StopFailure additionally carries the second `--rewake-waiter` entry — see the row above.) |
...
| PreToolUse (Bash) | `hooks/notion-write-guard.sh` | Denies ad-hoc REST writes to `api.notion.com` (a write verb via `-X`/`--request`/`method=`/a client `.post(`-class call), so workspace writes go through the tasks sync tool's clobber guard and dry-run instead. `apart-tools/tasksync/` and `apart-tools/notion-mirror/` invocations are allow-listed (the clone root alone is deliberately not enough; the old in-workspace `_tools` paths were retired 2026-08-16 with the trees). Matches the **raw** command string — no heredoc or quote stripping, inverting `no-truncation.sh`'s ordering gotcha, because here the URL sits inside quotes and an inline heredoc is a real write vector. Keys on the host, so SDK calls (`notion_client`) never match and the MCP tools stay gated by `permissions.ask` instead: this is friction, not a boundary, and the deny message says so. That message also names the script, the settings block, the sanctioned path and the way to disable it — its predecessor was reverted by its own author the next morning as an unexplained obstacle. Never logs the command (they carry integration tokens); journald records only that a deny fired. Matcher table: `hooks/notion-write-guard.test.sh` (25 cases, not deployed). |
| PreToolUse (Write\|Edit) | `hooks/roughdraft-write-guard.sh` | Snapshots the pre-write bytes of any CriticMarkup-bearing `.md` file before an agent overwrites it. **Snapshot-only: always `exit 0`, never any stdout.** Reviewed markdown (`{>>…<<}` comments, `{++…++}` suggestions, YAML endmatter) is ordinary text to every other tool, so a second session writing from a stale copy erases it and the only recovery was an hourly whole-filesystem btrfs snapshot. Emitting a decision was deliberately rejected: `permissionDecision: "ask"` is a hard **deny** wherever no prompt surface exists (background subagents, `claude -p`, `dontAsk`) — most of this box's traffic, including the review-resolution happy path — and that is the shape that got `no-truncation.sh` reverted by its own author. Bail ladder, cheapest first: no stdin → one `jq` for `tool_name`+`file_path` → `Write`/`Edit` only → `*.md` → not already inside a `.roughdraft-history/` segment (matched case-insensitively, as the server does, so a `.RoughDraft-History/` path can't be snapshotted by one and refused by the other) → readable regular file → a fixed-string marker grep (`{#` anchors alone don't count). Snapshots land in **`<dir>/.roughdraft-history/v1/<stem>/<UTC ts>--p<pid>--hook.md`** — a frozen cross-repo format also implemented in the Roughdraft repo's `packages/server/src/checkpoint-store.ts` and specified in its `docs/spec/history-sidecar.md`; recover with `roughdraft history <file>` or by copying out of that sidecar directly. Content-only dedup against the newest entry, a 90s debounce **against the hook's own `--hook` snapshots only**, so an agent's edit burst coalesces into one entry instead of churning the ring while another writer's fresh `--save`/`--review` entry never suppresses a capture (that would silently drop everything the reviewer typed after the save), prune to 50 oldest-first while **pinning the newest `--review` entry** (the last reviewed state is what the whole facility protects). Only files whose names match the id grammar count as snapshots at all: a hand-placed `zzz.md` in the sidecar is never the dedup baseline, never counts toward the cap, and is never evicted, a `.gitignore` containing `*` written into the sidecar (else `git clean -fd` deletes it and `git add -A` publishes deleted content), and `[ -L ]` refusal at all three dir levels (a symlinked sidecar is arbitrary-directory file creation triggered by whoever next edits the document). Prune never uses `ls \| xargs` — a planted space-in-name snapshot would otherwise delete same-named files in the agent's cwd. Every refusal that means a document silently gets no protection — an uncreatable sidecar, an unlistable leaf, a symlinked dir — logs a reason; the probe can't see those, since it fires at its own fixture and so only proves the hook works *somewhere*. Logs the file *path* only, never content: reviewed documents hold whatever was under review. Does not source `_hook-env.sh` (fires on every Write and Edit). Tests: `hooks/roughdraft-write-guard.test.sh` (not deployed; run it for the case list rather than trusting a count here); liveness watched by `health-check-apps.sh`, which checks three separate deaths: the script still works (one fixture per CriticMarkup marker, executed through its shebang, failing on no snapshot or on any stdout), it is still **wired** (a `PreToolUse` entry whose command names it and whose matcher covers both `Write` and `Edit` — the likeliest death, since `settings.json` is edited live rather than deployed), and it has not **disappeared** since it last worked (a `state/roughdraft-hook-deployed` marker, so post-deploy absence stops reading as pre-deploy silence; before that marker exists the probe is entirely silent). **Rollout:** hook config is read at **session start** — a fresh `roost-apply push files/hooks/roughdraft-write-guard.sh` reaches only new sessions, and removing the entry likewise needs a restart to take effect. The `settings.json` entry is wired **edit-live-then-backport** (edit `~/roost/claude/settings.json`, copy back into `files/settings.json`); never blanket `roost-apply push files/settings.json` — that file is runtime-rewritten and a push reverts whatever the repo hasn't learned yet. |

All these scripts source `_hook-env.sh` from `../lib/` (except `reflect.sh`, which just cats a prompt file; the UserPromptSubmit hook, which is the `scripts/session.sh` CLI reused as a hook; and `notion-write-guard.sh` and `roughdraft-write-guard.sh`, which fire on *every* Bash / *every* Write-Edit call and so cannot afford the `tailscale ip` subprocess, runtime-dir mkdir and per-exit `logger` that sourcing costs) which provides `hook_json()` for parsing Claude Code's JSON input, `ntfy_send()` for notifications (with journald fallback), `rate_limit_ok()` to prevent notification floods, and journald logging via `logger -t "$_HOOK_TAG"` (tags: `roost/<script-name>`).

Scheduled jobs (`scheduled/`, cron-triggered via `cron-roost`; not Claude Code events):
- `health-check.sh` -- Checks Ollama, Caddy, ntfy, Tailscale, cloudflared, disk, btrfs unallocated space (both filesystems; <5GiB alerts — df % misses this, and unallocated=0 with a full metadata pool is what force-flips the fs read-only, seen 2026-08-19 at df=75%); hard failures bundle into a high-priority `Service health alert`, cooled down by failure-set hash (notify on set change, else at most hourly). Soft signals (sustained swap >3GB high-priority, pending reboot via `/var/run/reboot-required` default-priority) send their own ntfy with per-event cooldowns (swap: 1h; reboot: 7d reminder, re-arms on new mtime). Sources `health-check-apps.sh` if present for app-specific checks.
...
- `btrfs-balance.sh` -- Weekly data-chunk balance (Sunday 2:30am, `/` + `/mnt/roost-data`): `-dusage=50 -dlimit=30` returns slack trapped in part-empty data chunks to the unallocated pool so the metadata pool can keep growing (the 2026-08-19 read-only flip was metadata ENOSPC with unallocated exhausted). Data chunks only, never metadata. ntfy on failure; logs before/after unallocated to `journalctl -t roost/btrfs-balance`.
- `auto-update.sh` -- Weekly updates (Sunday 3am) with btrfs snapshot before, ntfy summary after. Safeguards: 7-day release cooldown, major version guard (blocked and reported via ntfy). Updated tools: Claude Code, Codex CLI, claude-code-tools, aichat-search, claude-code-transcripts, Go, fnm, Node.js LTS, uv, gitleaks, dufs, rclone, PrivateBin, acme.sh, rodney, OS packages. Logs: `journalctl -t roost/auto-update`.
...
| App health checks | `~/roost/claude/scheduled/health-check-apps.sh` | Sourced by `health-check.sh` if present; uses same `check()` and `check_service()` helpers. Deployed from `files/travel/travel-health.sh` (currently hosts travel-vpn + PrivateBin checks and the roughdraft-write-guard liveness probe); append additional app-specific checks there rather than overwrite. |
