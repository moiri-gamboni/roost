...
- **`files/`** — everything deployed to the server (detail: `files/CLAUDE.md`)
  ...
  - `scripts/` — user CLIs → `claude/scripts/`, symlinked into `~/bin`: `roost-apply`, `roost-net`, `sym` (`files/scripts/CLAUDE.md`)
...
│   ├── scripts/            User CLIs → ~/bin (roost-apply, roost-net, agent-worktree, sym)
