...
│   ├── hooks/              Event hooks (notify, statusline, shellcheck-edit, notion-write-guard, truncation-guard (unwired), fork-context-guard, redelegation-guard, subagent-context, agent-browser-session, conflict-watch-hook)
...
| PreToolUse (every tool) · PostToolUse (writing tools) · UserPromptSubmit | `hooks/conflict-watch-hook.sh` | The conflict watch, session side. It warns and never prompts: when another open session holds a unit (a task folder, a plans/notes/data entry, a whole repo) because it wrote there, the first Edit/Write into it, the first Bash command naming it, and the first repo-wide git command that would change files that session wrote there are denied once with a warning (holder named, busy/idle, last write), and the retry passes. Writes that happen anyway (seen by the daemon) come back as "stop and ask the user", and the holder hears about them too. The model's options: ask the user in the conversation, SendMessage the holder, `agent-worktree isolate <repo>` for code; `conflict-watch allow <unit>` once the user says go ahead, `conflict-watch release` when done in a unit. Silent while the daemon is down |
