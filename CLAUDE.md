...
- **`files/`** -- Config files and templates deployed to the server
  ...
  - `laptop/` -- Scripts and systemd units designed to run on the laptop, not the server. Each component has a self-contained `install-*.sh` that reads `.env` and handles install + unit rendering + enable in one step.
    ...
    - `roost-travel.sh` + `roost-travel.service` + `install-travel.sh` -- Laptop-side sing-box tunnel. `install-travel.sh` is a one-shot installer (sing-box CLI + cfst/CloudflareSpeedTest + wrapper + systemd unit + config fetch). Usage: `roost-travel {on|off|status|logs|config|ips}`; `on`/`off` toggle both running state and enabled state (persistence across reboot); `ips` runs cfst against CF's published prefixes from the laptop's network and pushes the top 5 IPs (by latency) to the server's `~/roost/travel/cf-preferred-ip` for Path A's urltest pool.
