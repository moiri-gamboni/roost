...
│   ├── hooks/              Event hooks (notify, statusline, shellcheck-edit, notion-write-guard, beeper-send-guard, truncation-guard (unwired), fork-context-guard, redelegation-guard, subagent-context, agent-browser-session) + session-switch-notify (the session CLI's push on a login switch)
...
| `agent -N` / `--no-worktree` | Launch directly in the directory, no worktree. Per-repo: `git config agent.noWorktree true`; set on a *nested* repo it means "share me live into every session tree" instead of a per-session checkout (the Notion mirror at `~/roost/apart-research/mirrors/notion`) |
