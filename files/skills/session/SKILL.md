...
Auto-resume is armed by the harness, not the model: a second UserPromptSubmit hook entry (`session --rewake-waiter`, `asyncRewake: true`) backgrounds itself and, once a window crosses `USAGE_WARN_PCT` (default 90), becomes a waiter that wakes the session — even from idle — when the window resets or the login switches (exit 2 delivers its message as a system reminder through the task-notification channel). So a cap is never a reason to stop, wind down, or hold back parallel work: worst case is a pause the armed waiter rides out, and the ⚠ advisory the per-turn hook injects at the threshold says exactly that — one message even when both windows warn, identical whatever the window or reset distance, with only the lead sentence naming which window(s) crossed. Manual `session --wait 5h`/`week`/`guard` in the background (Bash `run_in_background`) still works the same way for explicit pacing, and also exits within ≤5m of a mid-wait login switch — the wait was armed against the old login's window, so the switch is the wake-up. Complementary refresh notice: once a warned window's reset time passes — or the login switches (`session account use`) to an account below the threshold — the hook injects a single one-line "the window has reset / the login switched — work can continue" notice on the session's next turn (no usage breakdown).
