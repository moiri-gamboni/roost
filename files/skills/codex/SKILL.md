...
**Background runs need `< /dev/null`.** `codex exec` reads stdin even when the prompt is a positional argument, so a backgrounded invocation with an open stdin pipe hangs forever on `Reading additional input from stdin...` — looking, from the outside, exactly like a long run. Just add `< /dev/null` to the Bash `run_in_background: true` call — that keeps the harness's completion notification, which is the whole point of backgrounding. Do **not** wrap it in `setsid nohup … &`: detaching makes the shell exit immediately, so the notification fires on the wrapper rather than the work, and you are back to polling. Relatedly, never `pkill -f "codex exec"`: the pattern matches the wrapper shell issuing it, so the command kills itself.
