...
```bash
pdftotext "$pdf" - | claude -p --model opus --effort medium \
...
- Medium effort is the right depth for per-paper summarization. Save the interactive session for the passes that need cross-paper judgment: drawing links, clustering, synthesis — and run those over the *notes*, not the PDFs (orders of magnitude less input).
