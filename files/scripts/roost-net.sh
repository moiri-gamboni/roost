...
# Probe the host's *underlay* — i.e. raw eth0 → internet, NOT routed through
# wg-proton. After a profile swap, this catches the class of bug that took
# down Tailscale: a misconfigured profile that hijacks all non-VPN traffic
# into a dead tunnel would fail this check even when wg-proton itself looks
# healthy. Cloudflare's edge (1.1.1.1/cdn-cgi/trace) is anycast + always-up;
# three short attempts ride out transient blips without blocking the swap.
probe_underlay() {
    local attempt
    for attempt in 1 2 3; do
        if curl -sf --max-time 4 https://1.1.1.1/cdn-cgi/trace >/dev/null; then
            return 0
        fi
        sleep 1
    done
...
# Validate a raw Proton profile source has the WG essentials. Run before any
# destructive action (stopping the live tunnel, overwriting the live config) so
# a malformed file fails closed without taking down the egress.
validate_proton_source() {
    local profile="$1" source="$2"
    sudo test -f "$source" || die "Profile '$profile' source missing at $source"
    local missing=() key
    for key in PrivateKey Address PublicKey Endpoint AllowedIPs; do
        sudo grep -qE "^[[:space:]]*${key}[[:space:]]*=" "$source" \
            || missing+=("$key")
    done
    if [ ${#missing[@]} -gt 0 ]; then
        die "Profile '$profile' at $source missing required directives: ${missing[*]}"
    fi
}

# Generate /etc/wireguard/wg-proton.conf from a raw profile source, stripping any
# wg-quick lifecycle directives the source carries (Table/PostUp/PreDown/DNS) and
# injecting the roost-canonical values exactly once. Profiles drop straight from
# account.protonvpn.com/downloads with no hand-editing; correctness no longer
# rides on the user remembering to copy the template's four lines into each one.
# An active-profile marker is embedded so list-mode can identify which source
# the live config was synthesized from.
synth_proton_conf() {
    local profile="$1" source="$2" live="$3" tmp
    # Snapshot the prior synthesized config so the post-start health probe can
    # roll back to it on failure. Only back up regular files: legacy symlinks
    # (pre-synth layout) point at sources that may be missing Table=off etc.,
    # so restoring them would reintroduce the original bug.
    if sudo test -f "$live" && ! sudo test -L "$live"; then
        sudo cp -f "$live" "${live}.prev"
    fi
    tmp=$(sudo mktemp /etc/wireguard/.wg-proton.synth.XXXXXX)
    {
        printf '# roost-active-profile: %s\n' "$profile"
        printf '# Generated by roost-net from %s — DO NOT EDIT.\n' "$source"
        sudo awk '
            /^[[:space:]]*(Table|PostUp|PreDown|DNS)[[:space:]]*=/ { next }
            /^[[:space:]]*\[Interface\][[:space:]]*$/ {
                print
                print "Table = off"
                print "DNS ="
                print "PostUp = /etc/roost-travel/proton-routing.sh up"
                print "PreDown = /etc/roost-travel/proton-routing.sh down"
                next
            }
            { print }
        ' "$source"
...
# Roll the live config back to the snapshot saved by synth_proton_conf and
# bring wg-quick up against it; if that fails, force vpn fully off. Always
# fires `die` at the end — the caller doesn't need to handle return — and
# always sends an ntfy alert so the user knows the swap was undone. Used by
# the post-start health probe in `vpn profile` when the new profile breaks
# egress or hijacks the underlay.
rollback_or_off() {
    local reason="$1"
    local live="/etc/wireguard/wg-proton.conf"
    local prev="${live}.prev"
    logger -t "$_HOOK_TAG" -p user.err "Auto-rollback triggered: $reason"
    sudo systemctl stop wg-quick@wg-proton || true
    # Belt-and-suspenders: if PreDown didn't fully unwind (e.g. legacy
    # config without PreDown), nuke our rules directly so the rollback
    # tunnel comes up against a clean slate.
    sudo /etc/roost-travel/proton-routing.sh down || true
    if sudo test -f "$prev"; then
        local prev_name
        prev_name=$(sudo awk '/^# roost-active-profile:/ {print $3; exit}' "$prev" 2>/dev/null || echo "previous")
        sudo cp -f "$prev" "$live"
        if sudo systemctl start wg-quick@wg-proton && probe_wg_egress >/dev/null; then
            ntfy_send -t "VPN profile rolled back" -p "high" \
                "Reverted to '$prev_name' after: $reason"
            die "Rolled back to '$prev_name': $reason"
        fi
        # Restore-start or its probe failed — fall through to forced off.
        sudo systemctl stop wg-quick@wg-proton || true
        sudo /etc/roost-travel/proton-routing.sh down || true
    fi
    ...
    ntfy_send -t "VPN forced off (rollback unavailable)" -p "max" \
        "No usable previous profile to restore. Reason: $reason"
    die "VPN forced off; rollback unavailable. $reason"
}
...
cmd_vpn() {
    ...
    case "$action" in
        on)
            ...
            # One-shot migration off the legacy symlink layout. Pre-synth
            # systems pointed wg-proton.conf at a hand-customized profile that
            # had to carry Table/PostUp/PreDown/DNS itself; missing those (as
            # with raw Proton downloads) silently broke the tunnel. Synth
            # replaces the symlink with a normalized regular file so the next
            # `vpn on` after upgrade can't reuse a malformed source.
            if sudo test -L /etc/wireguard/wg-proton.conf; then
                local tgt name
                tgt=$(sudo readlink -f /etc/wireguard/wg-proton.conf)
                case "$tgt" in
                    "$STATE_DIR"/proton-profiles/*.conf)
                        name=$(basename "$tgt" .conf)
                        logger -t "$_HOOK_TAG" "Migrating legacy symlink layout (profile=$name) to synthesized config"
                        validate_proton_source "$name" "$tgt"
                        synth_proton_conf "$name" "$tgt" /etc/wireguard/wg-proton.conf
                        ;;
                    *)
                        die "wg-proton.conf -> $tgt is outside $STATE_DIR/proton-profiles/. Activate a profile: roost-net vpn profile <name>"
                        ;;
                esac
            fi
            ...
            # Underlay sanity: confirms eth0 still reaches the internet
            # independently of the tunnel. With synth's Table=off this should
            # always pass; defense-in-depth against future routing regressions.
            if ! probe_underlay; then
                sudo systemctl disable --now wg-quick@wg-proton
                die "Underlay probe failed (eth0 → internet broken; route hijack?)"
            fi
        ...
        profile)
            ...
            local live="/etc/wireguard/wg-proton.conf"
            if [ -z "$profile" ]; then
                # List mode — the active marker lives in the synthesized live
                # config as a top-of-file comment. Fall back to readlink so
                # legacy symlink layouts still show an [active] tag pre-upgrade.
                local current=""
                if sudo test -f "$live" && ! sudo test -L "$live"; then
                    current=$(sudo awk '/^# roost-active-profile:/ {print $3; exit}' "$live" 2>/dev/null || true)
                elif sudo test -L "$live"; then
                    current=$(basename "$(sudo readlink -f "$live")" .conf 2>/dev/null || true)
                fi
            ...
            # Validate before touching the live tunnel. A bad source must not
            # take the VPN down — fail closed with vpn state unchanged.
            validate_proton_source "$profile" "$target"

            local was_on=false
            [ "$(read_state_file "$STATE_DIR/vpn" "off")" = "on" ] && was_on=true

            # Order matters when vpn is already on. systemctl restart reads the
            # live config for both down and up phases; if we synth before
            # stopping, wg-quick's PreDown invokes proton-routing.sh down with
            # the NEW endpoint, leaving the OLD endpoint's ip rule + iptables
            # exemption stranded. Stop with the OLD config first → OLD PreDown
            # cleans up → synth → start with NEW config → NEW PostUp installs
            # fresh state.
            if $was_on; then
                echo "Stopping wg-quick@wg-proton (clean up old profile state)..."
                sudo systemctl stop wg-quick@wg-proton || true
            fi

            synth_proton_conf "$profile" "$target" "$live"
            echo "Active Proton profile: $profile (synthesized at $live from $target)"

            if $was_on; then
                echo "Starting wg-quick@wg-proton with new profile..."
                if ! sudo systemctl start wg-quick@wg-proton; then
                    rollback_or_off "wg-quick failed to start with profile '$profile'"
                fi
                local ip
                if ! ip=$(probe_wg_egress); then
                    rollback_or_off "egress probe via wg-proton failed after swap to '$profile'"
                fi
                if ! is_external_egress "$ip"; then
                    rollback_or_off "egress $ip matches Hetzner after swap to '$profile' (leak)"
                fi
                # Underlay probe: catches the bug class that took Tailscale
                # down in May 2026 — a broken profile that hijacked all
                # non-fwmark traffic into a dead wg-proton tunnel passed the
                # egress probe (because xray's curl uses wg-proton by design)
                # but eth0's own traffic to the internet was dead. With synth
                # always installing Table=off this can't recur, but the probe
                # is cheap defense-in-depth against future regressions.
                if ! probe_underlay; then
                    rollback_or_off "underlay probe failed after swap to '$profile' (eth0 can't reach internet)"
                fi
...
Subcommands:
  ...
  vpn profile [name]         List or activate a Proton profile from /etc/roost-travel/proton-profiles/.
                             Synthesizes /etc/wireguard/wg-proton.conf (Table/PostUp/PreDown/DNS injected);
                             restarts wg if vpn=on (stops first to clean old endpoint state).
