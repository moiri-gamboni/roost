...
# rate_limits are account-global, so any session's render keeps this cache fresh.
_u="$HOME/roost/claude/usage"
[ -d "$_u" ] || mkdir -p "$_u"
printf '%s' "$input" > "$_u/.last-status.tmp" && mv -f "$_u/.last-status.tmp" "$_u/last-status.json"
