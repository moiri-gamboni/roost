#!/bin/bash
cat << 'EOF'
Before compacting, review this conversation for learnings worth preserving:

1. Search ~/memory/ via grepai for existing notes on topics covered
2. For each non-obvious discovery, bug fix, pattern, or dead end:
   - Does it pass the "6-month test"?
   - If a matching note exists: update it
   - If no match: write a new note using the format in CLAUDE.md
3. If a reusable procedure was discovered, create/update a skill in ~/.claude/skills/learned/
4. If a project-specific instruction emerged, update the project CLAUDE.md
EOF
