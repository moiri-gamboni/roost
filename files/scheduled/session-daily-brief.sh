...
# the model, never the full 8-day logs. Summarized by Opus (the `opus` alias) at
# medium effort, pushed as PLAIN TEXT via ntfy (phone apps
...
if ! brief=$(printf '%s' "$prompt" | timeout 900 claude -p --model opus --effort medium 2>"$tmp/err"); then
