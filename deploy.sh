...
# config.yml.base is the source of truth for the tunnel header; the
# assembled config.yml is generated by cloudflare-assemble.sh from .base
# plus per-app fragments in ~/roost/cloudflared/apps/*.yml. At first
# deploy there are no fragments, so the two files match.
remote "$ROOT_CMD mkdir -p /etc/cloudflared"
remote "$ROOT_CMD cp /home/$USERNAME/.cloudflared/${TUNNEL_ID}.json /etc/cloudflared/"
remote "$ROOT_CMD cp /home/$USERNAME/.cloudflared/config.yml /etc/cloudflared/config.yml.base"
remote "$ROOT_CMD cp /etc/cloudflared/config.yml.base /etc/cloudflared/config.yml"
remote "$ROOT_CMD chmod 700 /etc/cloudflared"
remote "$ROOT_CMD chmod 600 /etc/cloudflared/${TUNNEL_ID}.json /etc/cloudflared/config.yml.base /etc/cloudflared/config.yml"
