...
- **The bridge's `config.yaml` carries three hand edits**, kept across starts by `--no-override-config`: `network.gmail_oauth` (client ID and secret, `listener_address: "127.0.0.1:8765"`), `logging.min_level: info`, and `bridge.only_bridge_tags: [m.favourite]` (the fork tags every email chat low priority on creation, which would hide email in Beeper's Low Priority section and make the board read every thread as dismissed). A rebuilt bridge needs all three again.
